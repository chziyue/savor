...
export interface RequestRecord {
  ...
  requestBody?: string;
  responseBody?: string;
  filterMarkers?: string[];  // 过滤标记（如 ['🔒隐私', '🔑密钥']）
...
export class StatsDatabase {
  ...
  insertBatch(records: RequestRecord[]): number {
    const insert = this.db.transaction((items: RequestRecord[]) => {
      const stmt = this.db.prepare(`
        INSERT OR IGNORE INTO requests (id, timestamp, model, prompt_tokens, completion_tokens, total_tokens, duration, status, error_message, request_body, response_body)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
